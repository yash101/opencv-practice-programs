...
int run()
{
    ...
    while(true)
    {
        ...
        if(!input.empty())
        {
            ...
            Mat hsv;
            Mat backImage = imread("shark_bird.jpg");
            //cvtColor(input, hsv, CV_BGR2HSV);
            input.copyTo(hsv);
            int hl, sl, vl, hm, sm, vm;
            createTrackbar("hmin", project_name, &hl, 254, 0);
            createTrackbar("smin", project_name, &sl, 254, 0);
            createTrackbar("vmin", project_name, &vl, 254, 0);
            createTrackbar("hmax", project_name, &hm, 254, 0);
            createTrackbar("smax", project_name, &sm, 254, 0);
            createTrackbar("vmax", project_name, &vm, 254, 0);
            ...
            inRange(hsv, Scalar(hl, sl, vl), Scalar(hm, sm, vm), hsv);
            Mat thresh;
            threshold(hsv, thresh, 128, 255, CV_THRESH_BINARY);
            imshow("WIND2", hsv);
            input.copyTo(output, thresh);
            threshold(thresh, thresh, 64, 255, CV_THRESH_BINARY_INV);
            backImage.copyTo(output, thresh);
            ...
            if(key != -1)
            {
                cout << "Program>> Key code: " << (int) key << " pressed!" << endl;
            }
            //ESC key
            if((int) key == 27 || (int) key == 131099)
            {
                exit(EXIT_SUCCESS);
            } else {
